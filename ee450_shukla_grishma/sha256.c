/*
 * SHA-256 implementation adapted from a public-domain style reference implementation.
 * Exposes sha256_easy_hash_hex(data, len, out_hex64_plus_null) used by this project.
 */
...
void sha256_easy_hash_hex(const void* data, unsigned int len, char* hash) {
    ...
    for (i = 0; i < 32; ++i) {
        hash[i * 2] = hex[(digest[i] >> 4) & 0x0f];
        hash[i * 2 + 1] = hex[digest[i] & 0x0f];
    }
    hash[64] = '\0';
}
